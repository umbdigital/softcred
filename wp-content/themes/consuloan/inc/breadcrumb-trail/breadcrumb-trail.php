...
class Breadcrumb_Trail {
	...
	public function __construct( $args = array() ) {
		...
		$this->themesflat_do_trail_items();
	}
	...
	public function default_labels() {
		...
		$labels = array(
			'browse'              => esc_html__( 'Browse:', 'consuloan' ),
			'home'                => esc_html__( 'Home', 'consuloan' ),
			'error_404'           => esc_html__( '404 Not Found', 'consuloan' ),
			'archives'            => esc_html__( 'Archives', 'consuloan' ),
			/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
			'search'              => esc_html__( 'Search results for &#8220;%s&#8221;', 'consuloan' ),
			/* Translators: %s is the page number. */
			'paged'               => esc_html__( 'Page %s', 'consuloan' ),
			/* Translators: Minute archive title. %s is the minute time format. */
			'archive_minute'      =>esc_html__( 'Minute %s', 'consuloan' ),
			/* Translators: Weekly archive title. %s is the week date format. */
			'archive_week'        => esc_html__( 'Week %s', 'consuloan' ),
	 ...
	 * @return void
	 */
	public function themesflat_do_trail_items() {

		/* If viewing the front page. */
		if ( is_front_page() ) {
			$this->themesflat_do_front_page_items();
		}
		...
		/* If not viewing the front page. */
			...
			$this->themesflat_do_network_home_link();
			$this->themesflat_do_site_home_link();

			/* If viewing the home/blog page. */
			if ( is_home() ) {
				$this->themesflat_do_posts_page_items();
			}

			/* If viewing a single post. */
			elseif ( is_singular() ) {
				$this->themesflat_do_singular_items();
			}
			...
			elseif ( is_archive() ) {
				...
				if ( is_post_type_archive() )
					$this->themesflat_do_post_type_archive_items();

				elseif ( is_category() || is_tag() || is_tax() )
					$this->themesflat_do_term_archive_items();

				elseif ( is_author() )
					$this->themesflat_do_user_archive_items();

				elseif ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
					$this->themesflat_do_minute_hour_archive_items();

				elseif ( get_query_var( 'minute' ) )
					$this->themesflat_do_minute_archive_items();

				elseif ( get_query_var( 'hour' ) )
					$this->themesflat_do_hour_archive_items();

				elseif ( is_day() )
					$this->themesflat_do_day_archive_items();

				elseif ( get_query_var( 'w' ) )
					$this->themesflat_do_week_archive_items();

				elseif ( is_month() )
					$this->themesflat_do_month_archive_items();

				elseif ( is_year() )
					$this->themesflat_do_year_archive_items();

				else
					$this->themesflat_do_default_archive_items();
			}

			/* If viewing a search results page. */
			elseif ( is_search() ) {
				$this->themesflat_do_search_items();
			}

			/* If viewing the 404 page. */
			elseif ( is_404() ) {
				$this->themesflat_do_404_items();
			}
		}

		/* Add paged items if they exist. */
		$this->themesflat_do_paged_items();
	 ...
	 * @return void
	 */
	public function themesflat_do_rewrite_front_items() {
		global $wp_rewrite;

		if ( $wp_rewrite->front )
			$this->themesflat_do_path_parents( $wp_rewrite->front );
	}
	 ...
	 * @return void
	 */
	public function themesflat_do_paged_items() {
	 ...
	 * @return void
	 */
	public function themesflat_do_network_home_link() {
	 ...
	 * @return void
	 */
	public function themesflat_do_site_home_link() {
	 ...
	 * @return void
	 */
	public function themesflat_do_front_page_items() {
	 ...
	 * @return void
	 */
	public function themesflat_do_posts_page_items() {
		...
		if ( 0 < $post->post_parent )
			$this->themesflat_do_post_parents( $post->post_parent );
	 ...
	 * @return void
	 */
	public function themesflat_do_singular_items() {
		...
		/* If the post doesn't have a parent, get its hierarchy based off the post type. */
			...
			$this->themesflat_do_post_hierarchy( $post_id );

		/* Display terms for specific post type taxonomy if requested. */
		$this->themesflat_do_post_terms( $post_id );
	 ...
	 * @return void
	 */
	public function themesflat_do_post_parents( $post_id ) {
		$parents = array();
	 ...
	 * @return void
	 */
	public function themesflat_do_post_terms( $post_id ) {
	 ...
	 * @return void
	 */
	public function themesflat_do_post_hierarchy( $post_id ) {
		...
		if ( 'post' === $post_type ) {
			...
			$this->themesflat_do_rewrite_front_items();

			/* Map the rewrite tags. */
			$this->themesflat_map_rewrite_tags( $post_id, get_option( 'permalink_structure' ) );
		}
		...
		elseif ( false !== $post_type_object->rewrite ) {
			...
			if ( !empty( $post_type_object->rewrite['slug'] ) )
				$this->themesflat_do_path_parents( $post_type_object->rewrite['slug'] );
		}
	 ...
	 * @return void
	 */
	public function themesflat_get_post_types_by_slug( $slug ) {
	 ...
	 * @return void
	 */
	public function themesflat_do_term_archive_items() {
		global $wp_rewrite;
		...
		if ( false !== $taxonomy->rewrite ) {
			...
			$this->themesflat_do_path_parents( $taxonomy->rewrite['slug'] );
			...
			if ( $taxonomy->rewrite['slug'] ) {
				...
				if ( isset( $matches ) ) {
					...
					foreach ( $matches as $match ) {
						...
						$post_types = $this->themesflat_get_post_types_by_slug( $match );
	 ...
	 * @return void
	 */
	public function themesflat_do_post_type_archive_items() {
	 ...
	 * @return void
	 */
	public function themesflat_do_user_archive_items() {
		global $wp_rewrite;
		...
		if ( !empty( $wp_rewrite->author_base ) )
			$this->themesflat_do_path_parents( $wp_rewrite->author_base );
	 ...
	 * @return void
	 */
	public function themesflat_do_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'consuloan' ) ) );
	}
	 ...
	 * @return void
	 */
	public function themesflat_do_minute_archive_items() {
		...
		/* Add the minute item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'consuloan' ) ) );
	}
	 ...
	 * @return void
	 */
	public function themesflat_do_hour_archive_items() {
		...
		/* Add the hour item. */
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'consuloan' ) ) );
	}
	 ...
	 * @return void
	 */
	public function themesflat_do_day_archive_items() {

		/* Add $wp_rewrite->front to the trail. */
		$this->themesflat_do_rewrite_front_items();

		/* Get year, month, and day. */
		$year  = sprintf( $this->args['labels']['archive_year'],  get_the_time( _x( 'Y', 'yearly archives date format',  'consuloan' ) ) );
		$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'consuloan' ) ) );
		$day   = sprintf( $this->args['labels']['archive_day'],   get_the_time( _x( 'j', 'daily archives date format',   'consuloan' ) ) );
	 ...
	 * @return void
	 */
	public function themesflat_do_week_archive_items() {
		...
		$year = sprintf( $this->args['labels']['archive_year'],  get_the_time( _x( 'Y', 'yearly archives date format', 'consuloan' ) ) );
		$week = sprintf( $this->args['labels']['archive_week'],  get_the_time( _x( 'W', 'weekly archives date format', 'consuloan' ) ) );
	 ...
	 * @return void
	 */
	public function themesflat_do_month_archive_items() {
	 ...
	 * @return void
	 */
	public function themesflat_do_year_archive_items() {
	 ...
	 * @return void
	 */
	public function themesflat_do_default_archive_items() {
	 ...
	 * @return void
	 */
	public function themesflat_do_search_items() {
	 ...
	 * @return void
	 */
	public function themesflat_do_404_items() {
	 ...
	 * @return void
	 */
	function themesflat_do_path_parents( $path ) {
		...
		if ( !empty( $post ) ) {
			$this->themesflat_do_post_parents( $post->ID );
		}
	 ...
	 * @return array
	 */
	public function themesflat_map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format',  'consuloan' ) ) ) . '</a>';

				/* If using the %monthnum% tag, add a link to the monthly archive. */
				elseif ( '%monthnum%' == $tag )
					$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'consuloan' ) ) ) . '</a>';

				/* If using the %day% tag, add a link to the daily archive. */
				elseif ( '%day%' == $tag )
					$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'consuloan' ) ) ) . '</a>';
...
class bbPress_Breadcrumb_Trail extends Breadcrumb_Trail {
	...
	public function themesflat_do_trail_items() {
		...
		elseif ( bbp_is_topic_tag_edit() ) {
			...
			if ( true === $this->args['show_title'] )
				$this->items[] = esc_html__( 'Edit', 'consuloan' );
		}
		...
		elseif ( bbp_is_single_topic() ) {
			...
			$this->themesflat_do_post_parents( bbp_get_topic_forum_id( $topic_id ) );
		...
		elseif ( bbp_is_single_reply() ) {
			...
			$this->themesflat_do_post_parents( bbp_get_reply_topic_id( $reply_id ) );
		...
		elseif ( bbp_is_single_forum() ) {
			...
			if ( 0 !== $forum_parent_id)
				$this->themesflat_do_post_parents( $forum_parent_id );
